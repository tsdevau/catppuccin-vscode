...
class CustomUIColorError extends Error {
  constructor(key: string, value: string, message = "") {
    super(`Invalid value: "${value}" for key "${key}" in "tpstech-catppuccin.customUIColors".\n${message}`)
...
const parseCustomUiColor = (k: string, v: string): [string, number] => {
  const entry = v.split(" ")
  if (entry.length > 2) {
    throw new CustomUIColorError(k, v, 'Too many arguments, expected "colorName" or "colorName opacityValue".')
  }
  let opacityValue = 1
  if (entry.length == 2) {
    ...
    if (opacityValue < 0 || opacityValue > 1) {
      throw new CustomUIColorError(k, v, "Opacity value is not between 0 and 1.")
    }
  }
