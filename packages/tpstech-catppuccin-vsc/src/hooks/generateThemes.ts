...
const optEnvironment = optEnvironmentVariable ? JSON.parse(optEnvironmentVariable) : {}
...
const main = async () => {
  ...
  flavors.map((flavor) => {
    ...
    writeFile(path.join(repoRoot, `themes/${flavor}.json`), JSON.stringify(theme, undefined, 2))
  })
}
